...
class RoomPinSearchRepository @Inject constructor(val wordsDb: WordsDb) :
    ...
    override fun getUnitWords(unitWordIds: IntArray): Result<List<UnitWord>> {
        try {
            var result = wordsDb.pinWordDao().getWords(unitWordIds)
    ...
    override fun getSize(): Result<Int> {
        try {
            val result = wordsDb.pinWordDao().getMaxId()
    ...
    override fun getAll(): Result<List<UnitWord>> {
        try {
            var result = wordsDb.pinWordDao().getAll()
    ...
    override fun getNgrams(ngram: String, positions: IntArray): Result<List<Pair<Int,List<Int>>>> {
        try {
            var result = wordsDb.pin3gramDao().getNgrams(ngram,positions)
    ...
    override fun getNgram(ngram: String, position: Int): Result<List<Int>> {
        try {
            var result = wordsDb.pin3gramDao().getNgram(ngram,position)
    ...
    override fun getAllNgrams(): Result<List<Triple<String, Int, List<Int>>>> {
        try {
            var result = wordsDb.pin3gramDao().getAll()
