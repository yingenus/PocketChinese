...
import com.yingenus.pocketchinese.controller.fragment.train.TrainingFragment
import com.yingenus.pocketchinese.model.LanguageCase
import java.lang.RuntimeException
import java.util.*

class TrainActivity:SingleFragmentActivityWithKeyboard() {
    ...
    private class TrainActivityFragmentFactory(val lang : LanguageCase, val uuid: UUID, val block : Int) : FragmentFactory(){
        override fun instantiate(classLoader: ClassLoader, className: String): Fragment {
            if(className == TrainingFragment::class.java.name)
                return TrainingFragment(lang,uuid,block)
    ...
    private fun getFragment():Fragment{
        return supportFragmentManager.fragmentFactory.instantiate(classLoader,TrainingFragment::class.java.name)
    }
}