...
class MultipleLineProgressBar : FrameLayout, ProgressItem.OrientationChangedObserver {
    ...
    private fun init(attrs: AttributeSet?){
        ...
        if (attrs != null){
            ...
            val attribute = context.theme.obtainStyledAttributes(attrs,com.yingenus.pocketchinese.R.styleable.MultipleLineProgressBar,0,0)

            try {
                animate = attribute.getBoolean(com.yingenus.pocketchinese.R.styleable.MultipleLineProgressBar_mpb_animate, animate)
                animationDuration = attribute.getInteger(com.yingenus.pocketchinese.R.styleable.MultipleLineProgressBar_mpb_animationDuration, animationDuration)
                progressSize = attribute.getDimension(com.yingenus.pocketchinese.R.styleable.MultipleLineProgressBar_mpb_progressSize, progressSize)
                dividerSize = attribute.getDimension(com.yingenus.pocketchinese.R.styleable.MultipleLineProgressBar_mpb_dividerSize, dividerSize)
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        for (index in 0 until childCount){
            ...
            val childMeasureSpecW = MeasureSpec.makeMeasureSpec(Math.max(0,childWidth), MeasureSpec.EXACTLY)
            val childMeasureSpecH = MeasureSpec.makeMeasureSpec(Math.max(0,childHeight), MeasureSpec.EXACTLY)
