...
public class PinyinPocketKeyboard extends LinearLayout implements KeyboardView.OnKeyboardActionListener {
    ...
    public PinyinPocketKeyboard(Context context) {
        super(context);
        init();
    }

    public PinyinPocketKeyboard(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public PinyinPocketKeyboard(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init();
    }

    public PinyinPocketKeyboard(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private void init(){
        ...
        mQwerty =new Keyboard(getContext(), R.xml.qwerty);
        mCandidates_a =new Keyboard(getContext(),R.xml.pin_a);
        mCandidates_e =new Keyboard(getContext(),R.xml.pin_e);
        mCandidates_i =new Keyboard(getContext(),R.xml.pin_i);
        mCandidates_o =new Keyboard(getContext(),R.xml.pin_o);
        mCandidates_u =new Keyboard(getContext(),R.xml.pin_u);
        mCandidates_v =new Keyboard(getContext(),R.xml.pin_v);

        LayoutInflater inflater=(LayoutInflater) getContext().getSystemService(Context.LAYOUT_INFLATER_SERVICE);

        createCandidatesView(inflater);
        createInputView(inflater);

        mCandidatesView.setVisibility(INVISIBLE);

        LayoutParams keyboardParams=new LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,ViewGroup.LayoutParams.WRAP_CONTENT);

        setOrientation(LinearLayout.VERTICAL);
        addView(mCandidatesView,keyboardParams);
        addView(mInputView,keyboardParams);

    }

    private void createInputView(LayoutInflater inflater) {
        KeyboardView pinKeyboard=(KeyboardView)inflater.inflate(R.layout.keyboard_pin_view,null);
        mInputView =pinKeyboard;
        pinKeyboard.setOnKeyboardActionListener(this);
        pinKeyboard.setKeyboard(mQwerty);
    }

    private void createCandidatesView(LayoutInflater inflater) {
        KeyboardView toneKeyboard=(KeyboardView)inflater.inflate(R.layout.keyboard_tone_view,null);
    ...
    private void updateCandidatesView(){
        CharSequence text=getTextBeforeCursor(1);
        if (text.length()!=0){
    ...
    public void deleteListener(EditText editText){
        if (mListener==null) return;
        if (mListener==editText){
            mListener=null;
        }
    }
    ...
    private CharSequence getTextBeforeCursor(int before){
        if (mListener==null) return "";

        int start=mListener.getSelectionStart();
        if (before>start) before=start;
        return mListener.getText().subSequence(start-before,start);
    }

    private void setComposingText(CharSequence str){
        if (mListener==null) return;

        int start=mListener.getSelectionStart();
        mListener.getText().insert(start,str);
    }
    private void deleteSurroundingText(int before,int after){
        if (mListener==null) return;

        Editable editable=mListener.getText();
        int start=mListener.getSelectionStart();

        if (before>start) before=start;
        if (after>editable.length()-start) after=editable.length()-start;
        editable.delete(start-before,start+after);

    }

    private void setCandidatesViewShown(boolean show){
        if (show)
